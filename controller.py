    ...
    for item in notification_details:
        ...
        for centre in centres:
            ...
            for session in valid_sessions:
                key = session['date'] + '-' + session['vaccine'] + '-MIN-AGE-' + str(session['min_age_limit'])
                if not key in valid_centres.keys():
                    valid_centres[key] = []
                valid_centres[key].append(centre['name'] + '-' + str(session['available_capacity']) + '-slots')
